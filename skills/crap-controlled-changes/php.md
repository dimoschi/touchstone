...
- Methods in changed files that are **never exercised by any test** won't appear in Clover output at all, so the table will under-report them. This is the same blind spot as Go's `_test.go`-only coverage, but more pronounced because PHP tests are class-named rather than colocated. Mitigation: at least run the `Unit` suite so all unit-tested code paths fire; if a changed class genuinely has no test, write one (the skill's TDD-first rule already requires this).
...
- **A phase that produces no Clover report is a warning, not a hard failure** (unlike Go, which exits 4). If the baseline produced nothing, every method reads as `new` against an empty baseline. When gitignored PHP files are present the warning names them: `git stash --include-untracked` does not stash ignored files, so they survive from the current tree into the baseline. See the same section in `go.md` for the full explanation.
