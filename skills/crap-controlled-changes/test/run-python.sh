...
FIXTURE_DIR="$SKILL_DIR/test/fixture-python"
PARSER="$SKILL_DIR/lib/parse_python.py"

WORK="$(mktemp -d)"
trap 'rm -rf "$WORK"' EXIT

BASE_TSV="$(CRAP_REPO_ROOT="$WORK" CRAP_CHANGED_FILES="src/branchy.py" \
  python3 "$PARSER" "$FIXTURE_DIR/radon-baseline.json" "$FIXTURE_DIR/coverage-baseline.json")"
CUR_TSV="$(CRAP_REPO_ROOT="$WORK" CRAP_CHANGED_FILES="src/branchy.py" \
  python3 "$PARSER" "$FIXTURE_DIR/radon-current.json" "$FIXTURE_DIR/coverage-current.json")"

echo "--- baseline ---"; echo "$BASE_TSV"
echo "--- current ---";  echo "$CUR_TSV"

echo "$BASE_TSV" | grep -q 'src/branchy.py::simple'   || { echo "FAIL: baseline missing simple"; exit 1; }
echo "$BASE_TSV" | grep -q 'src/branchy.py::branchy'  || { echo "FAIL: baseline missing branchy"; exit 1; }
echo "$BASE_TSV" | grep 'branchy' | grep -q $'\t4\t'  || { echo "FAIL: baseline branchy CC != 4"; exit 1; }
echo "$BASE_TSV" | grep 'branchy' | grep -q $'\t4.0$' || { echo "FAIL: baseline branchy CRAP != 4.0"; exit 1; }
...
OUT="$(awk -v BASEFILE="$WORK/base.tsv" -F '\t' '
  ...
  FILENAME == BASEFILE { base_cc[$1]=$2; base_cov[$1]=$3; base_crap[$1]=$4; next }
  {
...
echo "--- joined ---"; echo "$OUT"

echo "$OUT" | grep -q 'branchy.*NEEDS_TESTS.*worsened' || { echo "FAIL: branchy should be NEEDS_TESTS worsened"; exit 1; }
