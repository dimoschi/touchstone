...
description: MUST be invoked BEFORE writing or editing any Go, PHP, or Python file in a repo that has opted into gating with a .crap-gated file at its root. This skill mandates TDD via superpowers:test-driven-development as its first step, targets CRAP (Change Risk Anti-Patterns) ≤6 soft / ≤8 hard on changed functions/methods, and runs a pre-commit per-function check via crap-check.sh. Creating or removing the .crap-gated marker is the repo owner's decision, never this skill's. If you are about to write Go, PHP, or Python code in a repo with a .crap-gated marker and have not yet invoked this skill, STOP and invoke it now.
---
...
**`UNSUPPORTED_LANGUAGE` is not a gate failure about your code.** The gate scores Go, PHP and Python. In a repo carrying `.crap-gated`, staged source in another language that has functions is refused with exit 2 rather than passed, because a gate that waves through what it cannot measure claims a guarantee it never checked. Do not route around it by committing outside `crap-commit.sh`. Surface it to the user with the three options the gate prints: exempt the paths in `.crap-gated` (gitignore-style patterns, one per line, committed so the team shares the rule — note this also stops all three gates, coverage, dead code and mutation, from measuring Go/PHP/Python under that path, not only from refusing another language there), add a `lib/crap-check-<lang>.sh` module, or drop the marker. Choosing between those is the user's call, not yours.
...
**In a worktree, name the repo explicitly.** `crap-check.sh`, `mutation-check.sh` and `deadcode-check.sh` all take an optional leading `<absolute-repo-path>` argument, the same one `crap-commit.sh` already requires (`crap-commit.sh <absolute-repo-path> -m "..."`). Pass your worktree's path there instead of relying on cwd; a session's working directory is not guaranteed to be the worktree, and a gate given no path measures whatever repo the cwd happens to resolve to, silently. Every gate prints the repo root and branch it resolved as the first line of its output on every code path (`--accept`, `--verify`, `--mark-scored`, and a plain run alike) — read that line to confirm it measured the tree you meant. A leading path that is not a directory, or exists but is not a git repository, refuses with exit 2 naming the path rather than falling back to cwd.
...
- **`MUTATION_UNMEASURED`** (exit 0): no mutants survived, but only because none were generated for the changed lines/functions in the first place (or every one that was is exempted, e.g. inside `func main()`), so nothing was actually verified. Read the module output above it for which file; a zero-mutant result is not evidence the change is covered, just that this tool found nothing to mutate there.
