...
executed before Implement and again after, so it must list only read-only,
deterministic checks -- never suggest adding one that writes to the tree or depends on
network state the second run cannot repeat.
