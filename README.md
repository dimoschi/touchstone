...
4. **Implement** — TDD via the skill, committing through `crap-commit.sh`, which runs both commit-time gates and refuses while either is red. Alongside it, every line under the target repo's own `## Checks` heading (in `AGENTS.md` or `CLAUDE.md`) is executed before this phase and again after: it must list only read-only, deterministic checks, since it is run twice against the same tree.
