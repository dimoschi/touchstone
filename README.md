...
6. **Review** — adversarial reviewers on distinct lenses, chosen by diff size. A finding only holds the run if its category is one of a closed blocking set (wrong result, crash, gate bypass, unmet criterion) and its reproducer, run for real, exits nonzero *with* a fixed marker line on its own in the output; nonzero without the marker means the reproducer itself failed to run, not that it demonstrated anything, and becomes a note instead. Everything else reaches the PR as a non-blocking note. Re-runs on commits any later phase adds, and from that point on a finding also has to fall inside what the new commits actually changed.
7. **Fix** — confirmed findings only, bounded rounds. A finding counts as fixed when its own reproducer exits 0 against the new commits, marker or not, never by a model's judgement of the diff. One whose reproducer instead errors (nonzero, no marker) stays open and the next round's brief carries its exit code and output, so the fixer chases a real failure rather than a demonstration nobody gave.
...
A run budget, derived from the same triage judgement (roughly 60k output tokens
plus 800 per estimated line of change, clamped 80k-500k), refuses any further
...
outweigh its actual code by more than 3:1 (`args.supportRatio` to raise it).
